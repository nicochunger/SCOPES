...
import numpy as np
from class_definitions import Merit, Target, Observation, Plan
...
def airmass(observation: Observation, max: float, alpha: float = 1.0, verbose: bool = False) -> float:
    """ 
...
def altitude(observation: Observation, 
             min: float = config.scheduling_defaults['telescope elevation limits'][0], # type: ignore
             max: float = config.scheduling_defaults['telescope elevation limits'][1]) -> float: # type: ignore
...
def culmination(observation: Observation, verbose: bool = False) -> float:
    ...
    altitude_prop = (current_altitude-observation.min_altitude)/(observation.max_altitude-observation.min_altitude)
    if verbose:
...
def cadence(observation: Observation, delay: TimeDelta, alpha: float, beta: float = 0.5) -> float:
    """
    ...
    """
    pct_overdue = (observation.start_time - (observation.target.last_obs + delay)).to(u.day).value/delay.to(u.day).value
    if pct_overdue <= 0:
